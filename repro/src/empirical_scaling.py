...
def write_artifacts(
    ...
    for claim in range(1, 6):
        ...
        _write_json(d / "independent_empirical_checker_output.json", independent)
        _write_json(d / "negative_control_empirical_output.json", negative)
        _write_json(d / "environment.json", env)
        _write_text(d / "command.txt", RUN_COMMAND)
        _write_text(
            d / "method_empirical.md",
